...
namespace ZCSharpLib.Features.NetWork
{
    public class DynamicBuffer
    {
        ...
        public int GetIdleCount() //获得剩余的字节数
        {
        ...
        public void ModifySize(int size) //设置缓存大小
        {
        ...
        {
            // 缓存不够需要重新申请
            if (count > GetIdleCount()) // 写入的字节数大于剩余的字节数
            {
                int totalSize = Buffer.Length - GetIdleCount() + count; // 现在需要的总字节数
                byte[] tmpBytes = new byte[totalSize];
                Array.Copy(Buffer, 0, tmpBytes, 0, DataCount); //复制以前的数据
                Buffer = tmpBytes;
            }
